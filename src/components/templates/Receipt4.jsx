          ...
          {items.map((item, index) => (
            <React.Fragment key={index}>
              <tr className="align-bottom">
                ...
                <td className="text-right text-sm">{formatCurrency(parseFloat(item.amount) || 0, selectedCurrency)}</td>
              </tr>
              <tr className="align-top">
                <td colSpan="2" className="text-left text-sm pb-2">
                  HSN Code: {item.description}
                </td>
                <td className="text-right pb-2">Total: {formatCurrency(parseFloat(item.total) || 0, selectedCurrency)}</td>
              </tr>
            </React.Fragment>
          ))}
        </tbody>
        ...
        <table className="w-full">
          ...
          <tbody>
            <tr>
              ...
              <td className="text-right">
                {formatCurrency(subTotal * (taxPercentage / 200), selectedCurrency)}
              </td>
            </tr>
            <tr>
              <td>SGST</td>
